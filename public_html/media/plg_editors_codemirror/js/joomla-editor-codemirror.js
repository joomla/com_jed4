...
customElements.define('joomla-editor-codemirror', class extends HTMLElement {
  constructor() {
    ...
    this.closeFullScreen = this.closeFullScreen.bind(this); // Append the editor script
  ...
  connectedCallback() {
    const that = this;
    this.checkElement('CodeMirror').then(() => {
      ...
      this.checkElement('CodeMirror', 'findModeByName').then(() => {
        ...
        window.CodeMirror.defineInitHook(editor => {
          ...
          const mode = window.CodeMirror.findModeByName(editor.options.mode || '') || window.CodeMirror.findModeByName(editor.options.mode || '') || window.CodeMirror.findModeByExtension(editor.options.mode || '');
