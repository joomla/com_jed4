...
<div>
    ...
    <p>&trade;</p>
</div>`,window.ShadyCSS&&window.ShadyCSS.prepareTemplate(a,"joomla-core-loader"),this.attachShadow({mode:"open"}),this.shadowRoot.appendChild(a.content.cloneNode(!0)),window.ShadyCSS&&window.ShadyCSS.styleElement(this)}connectedCallback(){this.style.backgroundColor=this.color,this.style.opacity=.8,this.shadowRoot.querySelector("div").classList.add("box")}static get observedAttributes(){return["color"]}get color(){return this.getAttribute("color")||"#fff"}set color(a){this.setAttribute("color",a)}attributeChangedCallback(a,b,c){switch(a){case"color":c&&c!==b&&(this.style.backgroundColor=this.color);break;default:}}}a.get("joomla-core-loader")||a.define("joomla-core-loader",b)})(customElements);