...
tinymce.PluginManager.add('jdragndrop', function (editor) {
  ...
  function uploadFile(name, content) {
    ...
    var url = "".concat(editor.settings.uploadUri, "&path=").concat(editor.settings.comMediaAdapter);
    var data = (_data = {}, _defineProperty(_data, editor.settings.csrfToken, '1'), _defineProperty(_data, "name", name), _defineProperty(_data, "content", content), _defineProperty(_data, "parent", editor.settings.parentUploadFolder), _data);
    Joomla.request({
      url: url,
      method: 'POST',
      ...
      onSuccess: function onSuccess(resp) {
        ...
        if (response.data && response.data.path) {
          ...
          var dialogClose = function dialogClose(api) {
            ...
            var altValue = dialogData.altText ? " alt=\"".concat(dialogData.altText, "\"") : altEmpty;
            var lazyValue = dialogData.isLazy ? ' loading="lazy"' : '';
            var width = dialogData.isLazy ? " width=\"".concat(responseData.width, "\"") : '';
            var height = dialogData.isLazy ? " height=\"".concat(responseData.height, "\"") : '';
            editor.execCommand('mceInsertContent', false, "<img src=\"".concat(urlPath, "\"").concat(altValue).concat(lazyValue).concat(width).concat(height, "/>"));
          };
