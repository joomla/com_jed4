    ...
    <div class="clearfix">
        <div class="js-stools-container-bar">
            <label for="filter_search" class="element-invisible"
                   aria-invalid="false"><?php echo Text::_('COM_JED_NEW_SEARCH_FILTER_SUBMIT'); ?></label>
            <div class="input-group mb-3">
				<?php echo $search_filter->input; ?>
                <div class="input-group-append">
                    <button class="btn btn-outline-success hasTooltip" title=""
                            data-original-title="<?php echo Text::_('COM_JED_NEW_SEARCH_FILTER_SUBMIT'); ?>">
                        <i class="icon-search"></i>
                    </button>
					<?php if ($filters): ?>
                        <button type="button"
                                class="btn hasTooltip btn-outline-info dropdown-toggle js-stools-btn-filter" title=""
                                data-original-title="<?php echo Text::_('COM_JED_NEW_SEARCH_TOOLS_DESC'); ?>">
							<?php echo Text::_('COM_JED_NEW_SEARCH_TOOLS'); ?>
                        </button>
					<?php endif; ?>
                    <button type="button" class="btn btn-outline-primary hasTooltip js-stools-btn-clear" title=""
                            data-original-title="<?php echo Text::_('COM_JED_NEW_SEARCH_FILTER_CLEAR'); ?>"
                            onclick="jQuery(this).closest('form').find('input').val('');">
						<?php echo Text::_('COM_JED_NEW_SEARCH_FILTER_CLEAR'); ?>
                    </button>
                </div>
            </div>
        </div>
